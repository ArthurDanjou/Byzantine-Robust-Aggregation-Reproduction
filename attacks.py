...
def a_little_is_enough_attack(
    honest_gradients: list[torch.Tensor], num_byzantine: int, z: float = 1.0, **kwargs
) -> list[torch.Tensor]:
    """Implements the a-little-is-enough attack (Baruch et al., 2019).

    Args:
    ----
        honest_gradients: List of honest node gradients.
        num_byzantine: The number of byzantine workers in the network.
    ...
    stacked_gradients = torch.stack(honest_gradients)
    ...
    return [malicious_gradient] * num_byzantine
...
def fall_of_empires_attack(
    ...
    **kwargs,
    ...
    """Implements the fall of empires attack (Xie et al., 2019).

    Args:
    ----
        honest_gradients: Non-empty list of honest gradients.
        defense_func: Function to use for the aggregation rule.
        num_byzantine: Number of Byzantine gradients to generate.
        epsilon: Fixed epsilon if positive,
            number of evaluations for best attack epsilon if negative.
        **kwargs: Ignored keyword-arguments.
